...
from repressilator_selfactivating import RepressilatorSA
...
folder1 = path.join(".", "repressilatorSACostOne", "exper0")      
folder2 = path.join(".", "repressilatorSACostTwo", "exper0")      				
...
model1 = RepressilatorSA(parameter_values, np.array(["transcription", "transcription", "hill", "translation", "rna_degradation", "protein_degradation", "Kd", "ar_factor", "ar_Kd"]), np.array([0, 0, 10, 150, 0, 0]), mode=0) 
model2 = RepressilatorSA(parameter_values, np.array(["transcription", "transcription", "hill", "translation", "rna_degradation", "protein_degradation", "Kd", "ar_factor", "ar_Kd"]), np.array([0, 0, 10, 150, 0, 0]), mode=1) 
...
numSamples = 9
if readFromFile:
	sampleNumbers2 = pickle.load(open("stochastic_samples_numbers_repressilatorSA", "rb"))
else:
	num2 = region2.shape[0]  
	sampleNumbers2 = np.random.choice(num2, numSamples, replace=False)   
	pickle.dump(sampleNumbers2, open("stochastic_samples_numbers_repressilatorSA", "wb+"))    
	
#plot few simulations (ode + ssa)
t = np.linspace(0, model2.T, model2.N)
if readFromFile:
	samples = pickle.load(open("stochastic_samples_repressilatorSA","rb"))     
else:	
	samples = region2[sampleNumbers2,:]
	pickle.dump(samples, open("stochastic_samples_repressilatorSA", "wb+"))   
...
if readFromFile:
	t_total_ode, Y_total_ode, ts_total_stochastic, Y_total_stochastic = pickle.load(open("stochastic_simulations_repressilatorSA", "rb"))  
...
if not readFromFile:
	pickle.dump((t_total_ode, Y_total_ode, ts_total_stochastic, Y_total_stochastic) ,open("stochastic_simulations_repressilatorSA", "wb+"))       	
...
if readFromFile:
	pers, amps = pickle.load(open("stochastic_persamps_repressilatorSA","rb")) 
...
if not readFromFile:	
	pickle.dump((pers, amps), open("stochastic_persamps_repressilatorSA", "wb+"))  
