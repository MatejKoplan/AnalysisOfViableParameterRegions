...
class RepressilatorSR: 
	...
	def repressilatorModelOde(self, Y, t, can):  
		...
		try:
			dmx = -deltaRNA*mx + alpha /(1 + math.pow(z/kd, n) + math.pow(x/kd, o)) + alpha0 
			dmy = -deltaRNA*my + alpha /(1 + math.pow(x/kd, n) + math.pow(y/kd, o)) + alpha0  
			dmz = -deltaRNA*mz + alpha /(1 + math.pow(y/kd, n) + math.pow(z/kd, o)) + alpha0  
	...
	def represilatorStochastic(self, can):
		...
		while t < self.T:
			...
			a[1] = alpha /(1.0 + math.pow(y_conc[5]/(kd*omega), n) + math.pow(y_conc[1]/(kd*omega), o))*omega 
			a[2] = alpha0*omega  
			
			a[3] = deltaRNA*y_conc[2] 
			a[4] = alpha /(1.0 + math.pow(y_conc[1]/(kd*omega), n) + math.pow(y_conc[3]/(kd*omega), o))*omega
			a[5] = alpha0*omega   
			
			a[6] = deltaRNA*y_conc[4] 
			a[7] = alpha /(1.0 + math.pow(y_conc[3]/(kd*omega), n) + math.pow(y_conc[5]/(kd*omega), o))*omega
			a[8] = alpha0*omega          
