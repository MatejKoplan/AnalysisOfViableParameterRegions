...
from repressilator_factors import Repressilator
...
parameter_values = {  "transcription": {"min": 0.01, "max": 50},  
				...
				"hill": {"min": 1, "max": 5},        
				"Kd": {"min": 0.01, "max": 250},
                                "activator": {"min": 0.0, "max": 5}
                                "inhibitor": {"min": 0.0, "max": 5}
				}  
...
folder1 = path.join(".", "repressilatorCostOne", "exper0")      
folder2 = path.join(".", "repressilatorCostTwo", "exper0")      				
...
model1 = Repressilator(parameter_values, np.array(["transcription", "transcription", "hill", "translation", "rna_degradation", "protein_degradation", "Kd"]), np.array([0, 0, 10, 150, 0, 0]), mode=0) 
model2 = Repressilator(parameter_values, np.array(["transcription", "transcription", "hill", "translation", "rna_degradation", "protein_degradation", "Kd"]), np.array([0, 0, 10, 150, 0, 0]), mode=1) 
...
if readFromFile:
	t_total_ode, Y_total_ode, ts_total_stochastic, Y_total_stochastic = pickle.load(open("stochastic_simulations_repressilator", "rb"))  
...
if not readFromFile:
	pickle.dump((t_total_ode, Y_total_ode, ts_total_stochastic, Y_total_stochastic) ,open("stochastic_simulations_repressilator", "wb+"))       	
